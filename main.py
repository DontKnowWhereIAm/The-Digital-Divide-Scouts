...
st.caption("Ask questions about the filtered districts and connectivity data.")

openrouter_key = os.getenv("OPENROUTER_API_KEY", "")

if not openrouter_key:
    st.warning("No OPENROUTER_API_KEY found in your .env file. AI Assistant is unavailable.")
else:
    # Build a concise data summary from the currently filtered GeoDataFrame
    if not filtered_gdf.empty:
        top_urgent = filtered_gdf.nlargest(5, "urgency_score")[["district_name", "urgency_score", "pct_no_internet", "median_income"]].to_string(index=False)
        summary_stats = (
            f"Filtered dataset: {len(filtered_gdf)} districts across {', '.join(selected_states)}.\n"
            f"Avg urgency score: {filtered_gdf['urgency_score'].mean():.3f} | "
            f"Avg % no internet: {filtered_gdf['pct_no_internet'].mean()*100:.1f}% | "
            f"Avg median income: ${filtered_gdf['median_income'].mean():,.0f}\n"
            f"Access level breakdown: {filtered_gdf['access_level'].value_counts().to_dict()}\n"
            f"Top 5 most urgent districts:\n{top_urgent}"
        )
    else:
        summary_stats = "No districts match the current filters."

    SYSTEM_PROMPT = (
        "You are a data analyst assistant for a Homework Gap Dashboard that tracks "
        "internet connectivity disparities in school districts. "
        "You help educators, policy makers, and advocates understand the data and suggest actions. "
        "Be concise, insightful, and actionable. Here is the current filtered data context:\n\n"
        + summary_stats
    )
    ...
    # Chat input
    user_input = st.chat_input("Ask about the data, e.g. 'Which districts need the most help?'")
    if user_input:
