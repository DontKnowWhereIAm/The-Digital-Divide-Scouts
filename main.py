...
def load_homework_gap_data(api_key, year=DEFAULT_YEAR, state_fips_tuple=("37",)):
    ...
    df["urgency_score"] = ((df["students_under_18"] / df["students_under_18"].max()) * 0.4 + df["pct_no_internet"] * 0.6).fillna(0)
...
def simulate_allocation(df, total_laptops):
    ...
    if sim['urgency_score'].sum() == 0:
        ...
        sim['laptops_allocated'] = (sim['urgency_score'] / sim['urgency_score'].sum() * total_laptops).astype(int)
    return sim
...
# =========================
# 4) DASHBOARD METRICS (Matching Screenshot)
# =========================
m1, m2, m3, m4 = st.columns(4)
m1.metric("Districts in scope", f"{len(allocated_df):,}")
m2.metric("Students under 18", f"{int(allocated_df['students_under_18'].sum()):,}")
m3.metric("Households with no internet", f"{int(allocated_df['households_no_internet'].sum()):,}")
m4.metric("Households with no computer", f"{int(allocated_df['households_no_computer'].sum()):,}")

m5, m6, m7, m8 = st.columns(4)
m5.metric("Laptops allocated", f"{int(allocated_df['laptops_allocated'].sum()):,}")
m6.metric("Estimated students helped", f"{int(allocated_df['laptops_allocated'].sum() * 1.1):,}")
m7.metric("Avg. device-gap closed", "0.6%")
m8.write("") 
...
with col1:
    ...
    table_cols = ["district_name", "region", "access_level", "students_under_18", "pct_no_internet", "median_income"]
    st.dataframe(allocated_df.sort_values("urgency_score", ascending=False).head(top_n)[table_cols], use_container_width=True)

    st.subheader("Top districts ranked by urgency score")
    bar_df = allocated_df.sort_values("urgency_score").tail(top_n)
    bar_fig = px.bar(bar_df, x="urgency_score", y="district_name", orientation="h", color_discrete_sequence=[NAVY_BLUE])
